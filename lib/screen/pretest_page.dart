...
class _PretestPageState extends State<PretestPage>
  ...
  // ─── Warna selaras dengan HomePage ───────────────────────────────────────
  static const Color _gradientStart = Color(0xFF1565C0); // lebih dalam
  ...
  static const Color _primaryAccent = Color(0xFF42A5F5);
  ...
  Future<void> _submitJawaban() async {
    if (_jawabanUser.length < _pretest!.soal.length) {
      final belum = _pretest!.soal.length - _jawabanUser.length;
    ...
    final jawabanList = _jawabanUser.entries
  ...
  // ── Header dengan gradient biru seperti HomePage ─────────────────────────
  Widget _buildHeader() {
    final bool inQuiz = _isStarted && !_sudahDikerjakan && _pretest != null;
    return Container(
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          inQuiz ? (_pretest?.judul ?? "Kuis") : "Pre Test",
                          style: const TextStyle(
                            fontSize: 18,
                            fontWeight: FontWeight.w800,
                            color: Colors.white,
                        ...
                        if (inQuiz)
                          Text(
                            "Soal ${_currentIndex + 1} dari ${_pretest!.soal.length}",
                            style: TextStyle(
                  ...
                  if (inQuiz)
                    Container(
                      ...
                      child: Text(
                        "${_jawabanUser.length}/${_pretest!.soal.length} dijawab",
                        style: const TextStyle(
                            fontSize: 12,
                            fontWeight: FontWeight.w700,
                            color: Colors.white),
  ...
  Widget _buildProgressBar() {
    final progress =
        _pretest == null ? 0.0 : (_currentIndex + 1) / _pretest!.soal.length;
    return Padding(
  ...
  Widget _buildBody() {
    ...
    if (_pretest == null) return _buildErrorState();
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...
          Row(
            children: [
              _infoCard(Icons.help_outline_rounded, "${_pretest!.soal.length}",
                  "Total Soal", _primary, _primaryLight),
              const SizedBox(width: 12),
              _infoCard(Icons.emoji_events_outlined, "${_pretest!.kkm}",
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                const Expanded(
                  child: Text(
                    "Pretest hanya dapat dikerjakan satu kali. Pastikan koneksi internet stabil sebelum memulai.",
                    style: TextStyle(
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                const Expanded(
                  child: Text(
                    "Pretest hanya bisa dikerjakan satu kali dan hasilnya telah tersimpan.",
                    style: TextStyle(
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          const Spacer(),
                          Text(
                            "${_pretest!.soal.length - _currentIndex - 1} soal lagi",
                            style: const TextStyle(
  ...
  Widget _buildQuizBottomBar() {
    final isLast = _currentIndex == _pretest!.soal.length - 1;
    final hasMinimap = _pretest!.soal.length > 1;
            ...
            if (hasMinimap) ...[
              SizedBox(
                ...
                child: ListView.builder(
                  ...
                  itemCount: _pretest!.soal.length,
                  itemBuilder: (ctx, i) {
                    final sid = _pretest!.soal[i].id;

...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _loadUserData() async {
    ...
    String? idKelas = prefs.getString('classroom_id');
    String? idJurusan = prefs.getString('classroom_id');
  ...
  Future<void> _fetchKelas(String idKelas) async {
    try {
      final response =
          await http.get(Uri.parse("${baseUrl}api/get-class/$idKelas"));
  ...
  Future<void> _fetchJurusan(String idKelas) async {
    try {
      final response = await http.get(
        Uri.parse("${baseUrl}api/get-major-by-classroom/$idKelas"),
      );
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        setState(() {
          namaJurusan = data['name'] ?? "Jurusan Tidak Ditemukan";
        });
      } else {
        setState(() {
  ...
  Future<void> loadProfilePhoto() async {
    final prefs = await SharedPreferences.getInstance();
    final String? avatarPath = prefs.getString('avatar_url');

    setState(() {
      fotoProfil = (avatarPath != null && avatarPath.isNotEmpty)
          ? "${baseUrl}storage/$avatarPath"
          : "https://via.placeholder.com/150";
    });
  }
  ...
  Future<void> _getImageFromCamera() async {
    try {
      ...
      if (photo != null) {
        ...
        _showCustomSnackBar('Foto tidak diambil.', isSuccess: false);
      }
    } catch (e) {
      _showCustomSnackBar('Terjadi kesalahan saat mengambil foto: $e',
          isSuccess: false);
    }
  }
  ...
  Future<void> _deleteProfilePhoto() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Set foto default (placeholder)
        await prefs.setString('avatar_url', 'https://via.placeholder.com/150');

        if (mounted) {
          setState(() {
            // Kalau kamu pakai variabel lokal
            fotoProfil = 'https://via.placeholder.com/150';
          });
        }

        _showCustomSnackBar(
            responseBody['message'] ?? 'Foto profil berhasil dihapus.',
            isSuccess: true);
      } else {
        final error = json.decode(response.body);
        _showCustomSnackBar(error['message'] ?? 'Gagal menghapus foto profil.',
            isSuccess: false);
      }
    } catch (e) {
  ...
  Future<void> _uploadImage(File imageFile) async {
    try {
      ...
      var request = http.MultipartRequest(
        'POST',
        Uri.parse("${baseUrl}api/update-profile-photo"),
      );
      ...
      if (response.statusCode == 200) {
        ...
        final String fullUrl = data['avatar_url'] ?? "";
        final String? relativePath = fullUrl.split("/storage/").length > 1
            ? fullUrl.split("/storage/")[1]
            : null;

        if (relativePath != null) {
          await prefs.setString('avatar_url', relativePath);

          setState(() {
            fotoProfil = "${baseUrl}storage/$relativePath";
          });

          _showCustomSnackBar('Foto profil berhasil diperbarui',
              isSuccess: true);
        } else {
          _showCustomSnackBar('Format URL tidak valid', isSuccess: false);
        }
      } else {
      ...
      print(stacktrace);
      _showCustomSnackBar('Terjadi kesalahan saat mengupload foto: $e',
          isSuccess: false);
    }
  }
